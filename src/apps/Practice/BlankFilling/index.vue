  ...
  <div class="flex flex-col justify-center items-center">
    ...
    <div v-if="fetchSuccess">
      <main class="flex flex-col flex-nowrap justify-center items-center h-96">
        <ul
          class="flex flex-row flex-nowrap justify-center items-start w-9/12 min-h-full h-80 mt-48"
        >
          <li>
            <title-board :title="practiceTitle"></title-board>
          </li>
          <li
            class="flex flex-col flex-nowrap justify-around items-center w-9/12 min-h-full ml-32 mr-32 border shadow-md"
          >
            <div class="pl-6 pr-5 pt-4">
              <div class="w-160">
                ...
                <span v-for="(Que, Num) in Ques.slice(1)" :key="Que">
                  ...
                  <input
                    ...
                    :class="{
                      'border-red-400':
                        (!flag[questionCount][Num] && ifJudge[questionCount]) ||
                        (userAnswers[questionCount][Num] === '' &&
                          ifJudge[questionCount]),
                      'border-green-500':
                        flag[questionCount][Num] && ifJudge[questionCount],
                    }"
                  />
              ...
              <div
                v-if="ifJudge[questionCount]"
                class="pt-10 flex-nowrap justify-around"
              >
                <span>{{ Tips.tips }}</span>
                <div
                  class="w-96 h-44 overflow-x-auto overflow-y-auto ml-32 mt-2"
                >
                  <div
                    v-for="(answer, num) in questions[questionCount].answer"
                    :key="answer"
                    class="ml-6 mt-0.5"
                  >
        ...
        <div
          class="flex flex-row flex-nowrap justify-around items-center w-9/12 h-24 mt-8 ml-44"
        >
          <ctl-button
            :name="prevQ"
...
export default {
  ...
  computed: {
    prevBtnDisabled: function () {
      return this.prevDisabled || this.uestionCount === 0
    }
  },
  methods: {
    getQuestion() {
